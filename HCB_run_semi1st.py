...
import os
import utils
from HCB_class import*
from HCB_ode_funs import ode_funs
...
states, rho0=model.generate_one(up_sites)
...
a=model.a
num=model.number
adag_a=model.generate_adag_a_ops()
...
e_ops=a+num # list of expectation values to evaluate
...
y0=expect(a+num+adag_a, rho0) # change index correspondingly
...
index=ode_class.flat_index(single_ops=['-','n'], double_ops=['+-'], index={}) 
...
y1_semi[index['n'],:]=y1_semi[np.diag(index['+-']),:]
...
y2_semi[index['n'],:]=y2_semi[np.diag(index['+-']),:]
...
def plot_evolution(show_type='z', show_ind=0):
    ...
    y_total_semi=np.append(y1_semi[index[show_type][show_ind]],y2_semi[index[show_type][show_ind]])
    plt.figure(show_ind)
    #plt.subplot(211)
    plt.plot(t_total, y_total, label="$Qutip Re <S^{}_{}>$".format(show_type, show_ind))
    plt.plot(t_total, y_total_semi, alpha=0.7, label="$semi-class Re <S^{}_{}>$".format(show_type, show_ind))
...
#    plt.axhline(y=-0.5, color='grey', linestyle='--')
    ...
    # plt.legend()
    plt.xlabel('t')
    plt.suptitle('HCB model L=%d, N=%d  eps=%.2f  t=%.2f W  $\gamma$=%.1f W' % (L,N, eps[show_ind],t,gamma[show_ind]))
    if save:
        plt.savefig(path+"/site {}.png".format(show_ind))    
for show_ind in range(N):
#    plot_evolution('+',show_ind)
    plot_evolution('n', show_ind)
